        ...
        return (
            <div className="side-panel">
                ...
                <div className="side-panel__btn-wrapper">
                    <Button onClick={(e) => clearState(e)} className={(currentUser.login === "admin") ? "side-panel__btn-clear" : "hidden"}>Сброс</Button>
                </div>
                
                <Form className="side-panel__form">
                <FormGroup className={(currentUser.login === "admin") ? "side-panel__form__group" : "hidden"}>
                ...
                <FormGroup className={(currentUser.login === "admin" && ChangedItem) ? "side-panel__form__group" : "hidden"}>
                    ...
                    <Input value={itemInSidePanel.resto} className="resto" onChange={(e) => changeResto(e.target.value)} type="select" name="resto" id="resto">
                        {allUsers.map(user => (user.name === "Admin") ? null : (<option key={user.name}>{user.name}</option>))}
                    </Input>
                </FormGroup>
                ...
                <FormGroup className={(currentUser.login === "admin" && ChangedItem) ? "side-panel__form__group" : "hidden"}>
                    ...
                    <div className="side-panel__form__group__category-wrapper">
                        ...
                        <Button onClick={(e) => categoriesModalOpen(e)} className={(currentUser.login === "admin") ? "side-panel__form__group__category-wrapper__btn" : "hidden"}>+</Button>
                    </div>
                    
                </FormGroup>
                
                
                <Button disabled={(currentUser.login === "admin") ? false : (ChangedItem === null) ? true : false} onClick={(itemInSidePanel._id.length === 0) ? (e) => {
